function confetti(el, params) {
	...
	for (let i = 0; i < c.flakes; i++) {
		let conId = `con${randInt(0, 1000)}fet${randInt(0, 1000)}ti${randInt(
			0,
			1000
		)}`;
		...
		if (c.colors == 'random' || !Array.isArray(c.colors)) {
			confettiColor = `rgb(${randInt(0, 255)}, ${randInt(
				0,
				255
			)}, ${randInt(0, 255)})`;
		} else {
		...
		switch (c.type) {
			...
			case 'xo':
				if (randInt(0, 1) == 1) {
					// O shape
					confettiType = `<circle id="${conId}" stroke="${confettiColor}" stroke-width="${randInt(
						1,
						3
					)}" fill="none" cx="0" cy="0" r="${randInt(
						4,
						7
		...
		${confettiSpin}
					...
					// X shape
					confettiType = `<path id="${conId}" fill="${confettiColor}" d="M -5 1 H -1 V 5 A 1 1 0 0 0 1 5 V 1 H 5 A 1 1 0 0 0 5 -1 H 1 V -5 A 1 1 0 0 0 -1 -5 V -1 H -5 A 1 1 0 0 0 -5 1"  filter="url(#blur${randInt(
						1,
						2
					)})">
		${confettiSpin}
				...
				break;
			case 'code':
				confettiType = `<text id="${conId}" fill="${confettiColor}" stroke-width="${randInt(
					1,
					3
				)}" fill="none" x="0" y="0" dominant-baseline="middle" text-anchor="middle" filter="url(#blur${randInt(
					1,
					2
				)})">${randInt(0, 1)}
		${confettiSpin}
				...
				break;
			case 'circle':
				confettiType = `<circle id="${conId}" fill="${confettiColor}" cx="0" cy="0" r="${randInt(
					4,
					7
				)}" filter="url(#blur${randInt(1, 2)})">
		${confettiSpin}
				...
				break;
			case 'flower':
				confettiType = `<g filter="url(#blur${randInt(
					1,
					2
				)})" id="${conId}">
			...
			<path fill="rgb(${randInt(0, 255)}, ${randInt(0, 255)}, ${randInt(
					0,
					255
				)})" d="M 0 -2 C -4 -10 4 -10 0 -2 L 0 2 C 4 10 -4 10 0 2 L 0 0 M 2 -2 C 4.242 -9.9 9.9 -4.242 2 -2 L -2 2 C -4.242 9.9 -9.9 4.242 -2 2 M -2 -2 C -9.9 -4.242 -4.242 -9.9 -2 -2 L 2 2 C 9.9 4.242 4.242 9.9 2 2 L 0 0 M 2 0 C 10 -4 10 4 2 0 L -2 0 C -10 4 -10 -4 -2 0 L 2 0" />
			<circle fill="rgb(${randInt(0, 255)}, ${randInt(0, 255)}, ${randInt(
					0,
					255
				)})" cx="0" cy="0" r="2.8" stroke="rgb(${randInt(
					0,
					255
			...
			case 'square':
				...
				confettiType = `<rect id="${conId}" fill="rgb(${randInt(
					0,
					255
				)}, ${randInt(0, 255)}, ${randInt(
					0,
					255
				)})" x="0" y="0" height="${hhww}" width="${hhww}" filter="url(#blur${randInt(
					1,
					2
				)})">
					${confettiSpin}
			...
			default:
				confettiType = `<rect id="${conId}" fill="rgb(${randInt(
					...
					0,
					255
				)})" x="0" y="0" height="${randInt(6, 18)}" width="${randInt(
					4,
					7
				)}" filter="url(#blur${randInt(1, 2)})">
					${confettiSpin}
		...
		animatedConfetti += `<g transform="translate(${randInt(
			ww * -0.3,
			ww * 0.3
		)} 0) scale(1.${randInt(0, 1)})">
		${confettiType}		
	</g>
	...
	let overlayId = `conf${randInt(0, 1000)}etti${randInt(0, 1000)}ver${randInt(
		0,
		1000
	)}lay`;
	...
	<path id="motionPath1" fill="none" stroke="none" d="M ${ww * 0.5} -${
		hh * 0
	} Q ${ww * 0.3} ${hh * 0.25} ${ww * 0.5} ${hh * 0.5} Q ${ww * 0.7} ${
		hh * 0.75
	} ${ww * 0.5} ${hh * 1.1}" />
	<path id="motionPath2" fill="none" stroke="none" d="M ${ww * 0.5} -${
		hh * 0
	} Q ${ww * 0.7} ${hh * 0.25} ${ww * 0.5} ${hh * 0.5} Q ${ww * 0.3} ${
		hh * 0.75
	} ${ww * 0.5} ${hh * 1.1}" />
	${animatedConfetti}
</svg>`;
